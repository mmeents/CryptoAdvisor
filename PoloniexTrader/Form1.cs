  ...
  public partial class Form1 : Form {
    ...
    public async void DoLoadMarketsAsync() {
      ...
      foreach (KeyValuePair<string, ExchangeTicker> x in xTask) {
        try {
          ...
          e.toAppLog("MarketSummary foreach on " + ms+" at "+es);
        } 
      }
    ...
    public void DoProcessBooks() {
      ...
      try {
        ...
        if (FocusedMarket.Asks.Keys.Count > 0) {
          decimal dTopAsk = FocusedMarket.Asks.Keys.First();
          if (FocusedMarket.CurrentLow > dTopAsk) {
            FocusedMarket.CurrentLow = dTopAsk;
          }
        }

        if (FocusedMarket.Bids.Keys.Count > 0) {
          decimal dTopBid = FocusedMarket.Bids.Keys.First();
          if (dTopBid > FocusedMarket.CurrentHigh) {
            FocusedMarket.CurrentHigh = dTopBid;
          }
        }
    ...
    public async void DoGetBalancesAsync() { 
      ...
      try {
        ...
        foreach (string sCur in xTask.Keys) {
          switch (sCur) {
            case sUSD:
              if (xTask[sCur].QuoteAvailable + xTask[sCur].QuoteOnOrders > 0.01) {
            ...
            case "BTC":
              ...
              break;
            default:
              if (xTask[sCur].QuoteAvailable + xTask[sCur].QuoteOnOrders > 0.0005) {
                Balances.AddUpdate(sCur, xTask[sCur]);
              } else {
                Balances.Remove(sCur);             
              }
      ...
      if (FocusedMarket is CMarket) {
        ...
        if (FocusedMarket.StopLongShort == "Long") {
          ...
          if (FocusedMarket.StopShort > FocusedMarket.CurrentHigh + FocusedMarket.CurrentTrueRange) {
            FocusedMarket.StopShort = FocusedMarket.CurrentHigh + FocusedMarket.CurrentTrueRange;
          }
        }        
        
        FocusedMarket.PrevPrice = FocusedMarket.PriceLast;
        FocusedMarket.CurrentHigh = FocusedMarket.PriceLast;
        FocusedMarket.CurrentLow = FocusedMarket.PriceLast;        
    ...
    private void Form1_MouseDown(object sender, MouseEventArgs e) {
      ...
      if ((iDisplayMode == 40) && (!handledClick)) {
        ...
        if ((e.X <= Convert.ToInt32(3 * f05Width)) &&
            (e.Y >= f05Height + (5 * sfText.Height)) &&
            (e.Y <= f05Height + (5 * sfText.Height) + (sfText.Height * iCount))
           ) {
          Int32 iY = e.Y - Convert.ToInt32(f05Height + 5 * sfText.Height);
    ...
    private void edLongShort_ValueChanged(object sender, EventArgs e) {
      if (edLongShort.Value == 0) {
        if (FocusedMarket.StopLongShort == "Short") {
          FocusedMarket.StopLongShort = "Long";
          FocusedMarket.StopLong = FocusedMarket.Bid - FocusedMarket.CurrentTrueRange;
        }
      } else {
        if (FocusedMarket.StopLongShort == "Long") {
          FocusedMarket.StopLongShort = "Short";
          FocusedMarket.StopShort = FocusedMarket.Ask + FocusedMarket.CurrentTrueRange;
        }
      }
    }
      ...
      if (this.edTotalBuy.InvokeRequired) {
        ...
        if (this.Visible) {
          ...
          if (iDisplayMode == 40) {
            ...
            edLongShort.Left = (f05Width * 4 ).toInt32T();
            edLongShort.Top = (f05Height * 5 - (sfText.Height * 7.5)).toDouble().toInt32T();
            if (!edLongShort.Visible) edLongShort.Visible = true;
    ...
    */
            ...
            if ((iDisplayMode == 40)&&(bHasMarkets)&&(FocusedMarket is CMarket)) {
              ...
              bg.Graphics.DrawString("Stops: ", fCur10, Brushes.WhiteSmoke,
                Convert.ToSingle(f05Width * 3), Convert.ToSingle(iTop - (sfText.Height * 6)));

              string sStop1 = "";
              string sStop2 = "";
              if (FocusedMarket.StopLongShort == "Long") {
                sStop1 = "Long Stop at " + FocusedMarket.StopLong.toStr8();
                sStop2 = "Stop Sell at " + (FocusedMarket.StopLong - 0.00000001m).toStr8();
              }
              bg.Graphics.DrawString(sStop1, fCur10, (FocusedMarket.StopLongShort == "Long" ? Brushes.Chartreuse : Brushes.WhiteSmoke), 
                Convert.ToSingle(f05Width * 3), Convert.ToSingle(iTop - (sfText.Height * 5)));

              bg.Graphics.DrawString(sStop2, fCur10, (FocusedMarket.StopLongShort == "Long" ? Brushes.Chartreuse : Brushes.WhiteSmoke),
                Convert.ToSingle(f05Width * 3), Convert.ToSingle(iTop - (sfText.Height * 4)));
              ...
              double dAvgSatPrice = dAvgPrice * 100000000;
              double dAvgPriceFee = FocusedMarket.PriceLast.toDouble() * 100000000 * 0.005;
              double dAvePriceAndFee = dAvgSatPrice + dAvgPriceFee+1;
              bg.Graphics.DrawString(FocusedMarket.BaseCurrency + " "+dTotalQ.toStr2()+" Avg "+ dAvgSatPrice.toStr2(), 
                fCur8, Brushes.White, Convert.ToSingle(iLeft), Convert.ToSingle(iTop));              
              bg.Graphics.DrawString(""+FocusedMarket.PriceLast.toSat() +" sat .5% = "+ dAvgPriceFee.toString()+" sat", 
              ...
              try {
                if (FocusedMarket.Asks != null) {
                  ...
                  bg.Graphics.DrawString("Stops: ", fCur10, Brushes.WhiteSmoke,
                     Convert.ToSingle(3 * f20Width + 2 * f05Width), Convert.ToSingle(iTop - (sfText.Height * 6)));

                  sStop1 = "";
                  sStop2 = "";
                  if (FocusedMarket.StopLongShort == "Short") {
                    sStop1 = "Short Stops " + FocusedMarket.StopShort.toStr8();
                    sStop2 = "Stop Buys at " + (FocusedMarket.StopShort + 0.00000001m).toStr8();
                  }
                  bg.Graphics.DrawString(sStop1, fCur10, (FocusedMarket.StopLongShort == "Short" ? Brushes.HotPink : Brushes.WhiteSmoke),
                    Convert.ToSingle(3 * f20Width + 2 * f05Width), Convert.ToSingle(iTop - (sfText.Height * 5)));

                  bg.Graphics.DrawString(sStop2, fCur10, (FocusedMarket.StopLongShort == "Short" ? Brushes.HotPink : Brushes.WhiteSmoke),
                    Convert.ToSingle(3 * f20Width + 2 * f05Width), Convert.ToSingle(iTop - (sfText.Height * 4)));                 
            ...
            if (Balances.Count > 0) {
              ...
              string sT = "" + Balances.TotalBTCValue.toStr8() + "BTC " + Balances.TotalUSDValue.toStr2() + " "+ sUSD;
